...
class Net(nn.Module):
    def __init__(self, state_dict=None, bitwidth=None, sa=True, keep_res="input", more_res="input"):
        ...
        self.conv1 = SC_Conv2d(1, 32, 3, 1, binary_weight=state_list[0], binary_bias=state_list[1],
                               bitwidth=bitwidth[0], keep_res=keep_res, more_res=more_res)
        self.conv2 = SC_Conv2d(32, 64, 3, 1, binary_weight=state_list[2], binary_bias=state_list[3],
...
def main():
    ...
    model = Net(state_dict=checkpoint, bitwidth=bitwidth_list, sa=True, keep_res="output" if args.ores else "input",
                more_res="weight" if args.wmres else "input").to(device)
